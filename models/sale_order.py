...
class ProductImage(models.Model):
	...
	image = fields.Binary("Custom Image")
	text_description = fields.Text(string="Text Description")
	image_type = fields.Selection([('front', 'Front'),
		('back', 'Back'),('sample', 'Sample'),], string="Image Position", default='sample')
...
class SaleOrder(models.Model):
	...
	def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
		...
		if kwargs.get('images_lines',[]):
			self.write({
				'images_lines' : kwargs.get('images_lines', [])
			})
		...
		if not order_line:
			...
			images_lines = self.images_lines.filtered(lambda x : x.product_id == order_line.product_id)
			for l_ in images_lines:
				l_.write({
		...
		if quantity <= 0:
			...
			images_lines = self.images_lines.filtered(lambda x : x.product_id == order_line.product_id and not x.sale_line_id)
			for l_ in images_lines:
				l_.write({
					'sale_line_id' : order_line.id
				})
